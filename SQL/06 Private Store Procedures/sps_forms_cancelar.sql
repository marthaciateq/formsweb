CREATE PROCEDURE sps_forms_cancelar
...
BEGIN
	...
	begin try
		...
		begin
			begin try
				execute sp_forms_workflow @idform, @idusuarioSesion, 'CANCELAR', @validador, @comentarios, null
			end try
			begin catch
				set @error = dbo.fn_error(error_message())
				if @error is null set @error = error_message()
